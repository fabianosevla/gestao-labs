        ...
        const loadLaboratories = async () => {
            try {
                ...
                if (data.length === 0) {
                    ...
                    const initialData = [
                        { name: "Lab 1", location: "Prédio A, Sala 101", responsible: "Dr. Silva", capacity: 20, status: "ativo" },
                        { name: "Lab 2", location: "Prédio B, Sala 202", responsible: "Dra. Souza", capacity: 15, status: "inativo" },
                    ];
        ...
        const loadMacroHardwares = async () => {
            try {
                ...
                if (data.length === 0) {
                    ...
                    const initialData = [
                        { name: "Servidor X", laboratoryId: 1, user: "João", status: "ativo" },
                        { name: "Máquina Y", laboratoryId: 2, user: "Maria", status: "inativo" },
                    ];
        ...
        const loadMicroHardwares = async () => {
            try {
                ...
                if (data.length === 0) {
                    ...
                    const initialData = [
                        { name: "Microscópio Z", laboratoryId: 1, user: "Ana", status: "ativo" },
                        { name: "Sensor W", laboratoryId: 2, user: "Pedro", status: "inativo" },
                    ];
        ...
        const loadSoftwares = async () => {
            try {
                ...
                if (data.length === 0) {
                    ...
                    const initialData = [
                        { name: "Software A", laboratoryId: 1, user: "Carlos", status: "ativo" },
                        { name: "Software B", laboratoryId: 2, user: "Luiza", status: "inativo" },
                    ];
    ...
    useEffect(() => {
        const loadAdminUsers = async () => {
            try {
                ...
                if (data.length === 0) {
                    // Dados iniciais para usuários administradores
                    const initialData = [
                        { name: "Admin 1", email: "admin1@lab.com", status: "ativo" },
                        { name: "Admin 2", email: "admin2@lab.com", status: "inativo" },
                    ];
                    await db.adminUsers.bulkAdd(initialData);
                    setAdminUsers(initialData);
                } else {
                    setAdminUsers(data);
                }
            } catch (error) {
