...
export default function Software() {
    const [softwareLabs, setSoftwareLabs] = useState([]);
    ...
    const handleSubmit = () => {
        setSoftwareLabs([...softwareLabs, formData]);
        setShowForm(false);
        setFormData({
            nome: "",
            descricao: "",
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-semibold text-gray-800">Software</h2>
                <button
            ...
            {showForm ? (
                <div className="bg-white p-6 rounded-lg shadow-lg">
                    <h3 className="text-xl font-semibold mb-4">Criar Software</h3>
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-gray-700 text-sm font-semibold mb-2">Lista de Softwares</label>
                            <input
                                type="text"
                                name="listaSoftwares"
                                value={formData.listaSoftwares}
                                onChange={handleChange}
                                className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                            />
                        </div>
                ...
                <table className="w-full bg-white shadow-md rounded-lg overflow-hidden">
                    <thead className="bg-gray-200">
                        <tr>
                            ...
                            <th className="px-4 py-2 text-left">Lista de Softwares</th>
                        </tr>
                    </thead>
                    <tbody>
                        {softwareLabs.length === 0 ? (
                            <tr>
                                <td colSpan="4" className="px-4 py-2 text-center text-gray-500">
                                    Nenhum laboratório de Software registrado.
                                </td>
                            </tr>
                        ) : (
                            softwareLabs.map((lab, index) => (
                                <tr key={index} className="border-t">
                                    <td className="px-4 py-2">{lab.nome}</td>
                                    <td className="px-4 py-2">{lab.descricao}</td>
                                    <td className="px-4 py-2">{lab.localizacao}</td>
                                    <td className="px-4 py-2">{lab.listaSoftwares}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
