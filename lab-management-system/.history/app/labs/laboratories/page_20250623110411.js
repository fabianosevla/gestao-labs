...
export default function Laboratories() {
    ...
    const [searchAcronym, setSearchAcronym] = useState(""); // NOVO CAMPO DE BUSCA
    ...
    const filteredLaboratories = laboratories.filter((lab) => {
        const matchesName = lab.name.toLowerCase().includes(searchName.toLowerCase());
        // Filtrar por sigla também
        const matchesAcronym = searchAcronym ? (lab.acronym?.toLowerCase().includes(searchAcronym.toLowerCase())) : true;
    ...
    if (loading || adminLoading) {
        return <div className="p-6 pt-28">Carregando laboratórios...</div>;
    }
    ...
    return (
        <div className="p-6 pt-28">
            ...
            <div className="mb-6 flex gap-4">
                ...
                {/* NOVO CAMPO DE BUSCA */}
                <div className="flex-1">
                    <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="searchAcronym">
                        Pesquisar por Sigla
                    </label>
                    <input
                        type="text"
                        id="searchAcronym"
                        value={searchAcronym}
                        onChange={(e) => setSearchAcronym(e.target.value)}
                        className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900"
                        placeholder="Digite a sigla para buscar"
                        autoComplete="off"
                    />
                </div>
            </div>
                ...
                <table className="w-full border-collapse bg-white shadow-md rounded-lg">
                    <thead>
                        <tr className="bg-gray-100">
                            ...
                            <th className="border p-3 text-gray-900 font-semibold text-left">Sigla</th> {/* NOVA COLUNA */}
                            <th className="border p-3 text-gray-900 font-semibold text-left">Capacidade</th> {/* NOVA COLUNA */}
                        ...
                        {filteredLaboratories.map((lab) => {
                            ...
                            return (
                                <tr key={lab.id.toString()} className="hover:bg-gray-50">
                                    ...
                                    <td className="border p-3 text-gray-700">{lab.acronym || "N/A"}</td> {/* EXIBIR SIGLA */}
                                    <td className="border p-3 text-gray-700">{lab.capacity || "N/A"}</td> {/* EXIBIR CAPACIDADE */}
