...
export default function EditVisitorUser() {
    ...
    useEffect(() => {
        const loadUser = async () => {
            ...
            const user = visitorUsers.find((u) => u.id === userId);
            console.log("Usuário encontrado:", user);

            if (!user) {
                console.log("Usuário não encontrado no estado atual, forçando recarregamento...");
                await refreshVisitorUsers(); // Força recarregamento se o usuário não for encontrado
                const refreshedUser = (await useVisitorUsers())[0].find((u) => u.id === userId);
                if (refreshedUser) {
                    setFormData({ ...refreshedUser });
                } else {
