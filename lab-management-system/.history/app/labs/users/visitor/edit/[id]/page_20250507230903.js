...
export default function EditVisitorUser() {
    ...
    const [hasRefreshed, setHasRefreshed] = useState(false); // Controle para evitar múltiplos recarregamentos
    ...
    useEffect(() => {
        const loadUser = async () => {
            ...
            if (!user && !hasRefreshed) {
                console.log("Usuário não encontrado, forçando recarregamento...");
                await refreshVisitorUsers();
                setHasRefreshed(true); // Marca que já recarregamos
                return; // Sai do useEffect para evitar loop
            }
            ...
            if (!user) {
                console.log("Usuário não encontrado após recarregamento, redirecionando...");
                router.push("/labs/users/visitor");
                return;
            }
        ...
        loadUser();
    }, [id, loading, router, refreshVisitorUsers, hasRefreshed]); // Removido visitorUsers das dependências
