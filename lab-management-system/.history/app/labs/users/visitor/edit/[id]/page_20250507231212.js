...
export function useLaboratories() {
    ...
    }, []); // Dependências vazias, pois não depende de props ou estado
...
export function useVisitorUsers() {
    ...
    const refreshVisitorUsers = useCallback(async () => {
        setLoading(true);
        try {
            const db = await openDB();
...
export function useMacroHardwares() {
    ...
    const refreshMacroHardwares = useCallback(async () => {
        setLoading(true);
        try {
            const db = await openDB();
...
export function useMicroHardwares() {
    ...
    const refreshMicroHardwares = useCallback(async () => {
        setLoading(true);
        try {
            const db = await openDB();
