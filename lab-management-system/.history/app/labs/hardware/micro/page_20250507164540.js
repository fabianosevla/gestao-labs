...
export default function MicroHardwares() {
    ...
    const handleDelete = async (id) => {
        ...
        await refreshMicroHardwares(); // Recarrega os dados do banco
    };

    const filteredMicroHardwares = microHardwares.filter((hw) => {
        const matchesName = hw.name
            .toLowerCase()
            .includes(searchName.toLowerCase());
        const matchesStatus = searchStatus
            ? hw.status.toLowerCase() === searchStatus.toLowerCase()
            : true;
                    ...
                    <tbody>
                        {filteredMicroHardwares.map((hw) => (
                            <tr key={hw.id.toString()} className="hover:bg-gray-50">
                                ...
                                <td className="border p-3 text-gray-700">{hw.userId || "Nenhum"}</td>
