...
export default function Micro() {
    ...
    const handleSubmit = () => {
        setMicroLabs([...microLabs, formData]);
        setShowForm(false);
        setFormData({
            nome: "",
            descricao: "",
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-semibold text-gray-800">Hardware - Micro</h2>
                <button
                    onClick={() => setShowForm(true)}
                    className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition"
                >
                    Criar Hardware Micro
                </button>
            </div>

            {showForm ? (
                <div className="bg-white p-6 rounded-lg shadow-lg">
                    <h3 className="text-xl font-semibold mb-4">Criar Hardware Micro</h3>
                        ...
                        {microLabs.length === 0 ? (
                            <tr>
                                <td colSpan="5" className="px-4 py-2 text-center text-gray-500">
                                    Nenhum laboratório Micro registrado.
                                </td>
                            </tr>
                        ) : (
                            microLabs.map((lab, index) => (
