...
// Importar todos os hooks de dados necessários
import {
  useLaboratories,
  useAdminUsers,
  useMacroHardwares, // Novo
  useMicroHardwares, // Novo
  useSoftwares,      // Novo
} from "../lib/storage";
...
export default function Home() {
  ...
  const [macroHardwares, loadingMacroHardwares] = useMacroHardwares(); // Novo
  const [microHardwares, loadingMicroHardwares] = useMicroHardwares(); // Novo
  const [softwares, loadingSoftwares] = useSoftwares();              // Novo
  ...
  // Verifica o estado de carregamento de TODOS os dados necessários
  if (loadingLaboratories || loadingAdminUsers || loadingMacroHardwares || loadingMicroHardwares || loadingSoftwares) {
    return <div className="p-6 pt-28 text-gray-700">Carregando dados...</div>;
  }

  // Filtra hardwares e softwares para o laboratório selecionado no modal
  const macroHardwaresForLab = selectedLaboratory
    ? macroHardwares.filter(hw => hw.laboratoryId === selectedLaboratory.id)
    : [];
  const microHardwaresForLab = selectedLaboratory
    ? microHardwares.filter(hw => hw.laboratoryId === selectedLaboratory.id)
    : [];
  const softwaresForLab = selectedLaboratory
    ? softwares.filter(sw => sw.laboratoryId === selectedLaboratory.id)
    : [];
    ...
    <div className="p-6 pt-28">
      ...
      {isLabDetailsModalOpen && selectedLaboratory && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-8 w-full max-w-lg shadow-xl relative animate-fade-in-up overflow-y-auto max-h-[90vh]"> {/* Adicionado overflow-y-auto e max-h */}
            ...
            <div className="space-y-4 text-gray-700">
              ...
              {/* HARDWARE MACRO ALOCADO */}
              <div className="border-t pt-4 mt-4">
                <h3 className="font-semibold text-lg text-gray-800 mb-2">Hardware Macro Alocado</h3>
                {macroHardwaresForLab.length === 0 ? (
                  <p className="text-gray-600 text-sm">Nenhum hardware macro alocado neste laboratório.</p>
                ) : (
                  <ul className="list-disc list-inside text-sm space-y-1">
                    {macroHardwaresForLab.map(hw => (
                      <li key={hw.id}>
                        <strong>{hw.name}</strong> (Qtde: {hw.quantity || 1}) - Modelo: {hw.model || "N/A"}
                        {hw.config1 && `, C1: ${hw.config1}`}
                        {hw.config2 && `, C2: ${hw.config2}`}
                        {hw.config3 && `, C3: ${hw.config3}`}
                      </li>
                    ))}
                  </ul>
                )}
              ...
              {/* HARDWARE MICRO ALOCADO */}
              <div className="border-t pt-4 mt-4">
                <h3 className="font-semibold text-lg text-gray-800 mb-2">Hardware Micro Alocado</h3>
                {microHardwaresForLab.length === 0 ? (
                  <p className="text-gray-600 text-sm">Nenhum hardware micro alocado neste laboratório.</p>
                ) : (
                  <ul className="list-disc list-inside text-sm space-y-1">
                    {microHardwaresForLab.map(hw => (
                      <li key={hw.id}>
                        <strong>{hw.name}</strong> (Qtde: {hw.quantity || 1}) - Especificação: {hw.specification || hw.model || "N/A"}
                      </li>
                    ))}
                  </ul>
                )}
              ...
              {/* SOFTWARE ALOCADO */}
              <div className="border-t pt-4 mt-4">
                <h3 className="font-semibold text-lg text-gray-800 mb-2">Software Alocado</h3>
                {softwaresForLab.length === 0 ? (
                  <p className="text-gray-600 text-sm">Nenhum software alocado neste laboratório.</p>
                ) : (
                  <ul className="list-disc list-inside text-sm space-y-1">
                    {softwaresForLab.map(sw => (
                      <li key={sw.id}>
                        <strong>{sw.name}</strong> - Status: {sw.status}
                      </li>
                    ))}
                  </ul>
                )}
