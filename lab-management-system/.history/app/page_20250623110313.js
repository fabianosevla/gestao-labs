...
export default function Home() {
  ...
  const [senderEmail, setSenderEmail] = useState(""); // NOVO ESTADO: E-mail do remetente
  ...
  const handleSubmitRequest = async (e) => { // Tornar a função async
    e.preventDefault();
    if (!selectedLabForRequest || !requestText || !senderEmail) { // Adiciona validação para senderEmail
    ...
    const emailSubject = `Nova Solicitação para o Laboratório ${lab.name} de ${senderEmail}`;
    const emailText = `Detalhes da Solicitação:\n\nRemetente: ${senderEmail}\nLaboratório: ${lab.name}\n\n${requestText}\n\nEnviado de: Sistema de Gerenciamento de Laboratórios UNIFEI`;
    const emailHtml = `
            <p><strong>Nova Solicitação:</strong></p>
            <p><strong>Remetente:</strong> <a href="mailto:${senderEmail}">${senderEmail}</a></p>
            <p><strong>Laboratório:</strong> ${lab.name}</p>
            <p><strong>Mensagem:</strong></p>
            <p>${requestText.replace(/\n/g, '<br>')}</p>
            <p><em>Enviado de: Sistema de Gerenciamento de Laboratórios UNIFEI</em></p>
        `;
    ...
    try {
      const response = await fetch('/api/send-email', {
        ...
        body: JSON.stringify({
          ...
          text: emailText,
          html: emailHtml,
          fromEmail: senderEmail, // ENVIA O E-MAIL DO REMETENTE PARA A API ROUTE
        }),
      });
      ...
      if (response.ok) {
        ...
        setRequestText("");
        setSenderEmail(""); // Limpa o campo do remetente
      } else {
        alert(`Erro ao enviar solicitação: ${data.message || 'Erro desconhecido'}. Verifique o console do servidor.`);
    ...
    <div className="p-6 pt-28">
          ...
          <ul className="space-y-3">
            {laboratories.map((lab) => (
              ...
              >
                ...
                {/* Ícone de Informação (exemplo com SVG do Heroicons) */}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-8 w-full max-w-lg shadow-xl relative animate-fade-in-up">
            ...
            <div className="space-y-4 text-gray-700">
              ...
              <p><strong>Sigla:</strong> {selectedLaboratory.acronym || "N/A"}</p> {/* NOVO CAMPO */}
              <p><strong>Capacidade:</strong> {selectedLaboratory.capacity || "N/A"}</p> {/* NOVO CAMPO */}
      ...
      <section className="bg-white shadow-md rounded-lg p-6 border border-gray-200 mt-8">
        ...
        <form onSubmit={handleSubmitRequest} className="space-y-4">
          ...
          {/* NOVO CAMPO: E-mail do remetente */}
          <div>
